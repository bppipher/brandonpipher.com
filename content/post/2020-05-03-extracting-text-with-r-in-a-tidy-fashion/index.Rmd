...
A short explanation of each step is provided below.  My regular expressions are on the beginner end due to inexperience, but I believe they handled this specific example well. The site [Regex101](https://regex101.com/) was quite helpful with handling the regex.
...
There were Python scripts available to manage this, but that felt just like cheating.  I tried a few tools like [SelectorGadget](https://cran.r-project.org/web/packages/rvest/vignettes/selectorgadget.html) to simplify scraping the relevant portions of the CSS but USNews' format was more unwieldly than it seemed capable of handling. There was also [this](https://github.com/tidyverse/rvest/issues/17) interesting, but old, `rvest` issue about this with a method of handling a different area of USNews rankings.  
...
save(usnewsrankings, file = paste0(dirname(rstudioapi::getSourceEditorContext()$path), "/usnewsrank.Rda"))
```
...
Should this process change the original copied data can be found [here](http://www.brandonpipher.com/post/2020-05-03-extracting-text-with-r-in-a-tidy-fashion/usnewsrank.Rda).
